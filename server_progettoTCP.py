...
def gestisci_comunicazione(conn):
    ...
    while True:
        ...
        #LETTURA------------------------------------------------------------------------------------------------------------------------------------------------------
        if(data=="R"):
            conn.send("In che tabella vuoi leggere: C=clienti, Z=zone di lavoro".encode())
        ...
        #INSERIMENTO------------------------------------------------------------------------------------------------------------------------------------------------------
            ...
            conn.send("In che tabella vuoi inserire: C=clienti, Z=zone di lavoro".encode())
            tabella = conn.recv(1024).decode()
            if(tabella=='C'):
...
def db_get(nome_dipendente, tab):
    ...
    if(tab=='C'):
        query = f"SELECT * FROM clienti_marco_santini WHERE nome = '{nome_dipendente}' "
    elif(tab=='Z'):
        query = f"SELECT * FROM zona_di_lavoro_marco_santini WHERE nome = '{nome_dipendente}' "
    print(query)
    cur.execute(query)
    dati = cur.fetchall()
    return dati
...
def db_inserisci(nome_dip, cogn_dip, tel_dip, tab, ore_dip):
    ...
    # si chiama una funzione di libreria passando i parametri di ricerca dell'utente. esempio controlla_caratteri(nome)
    if tab=='C':
        query = f"INSERT INTO `clienti_marco_santini`(`nome`, `cognome`, `telefono`) VALUES ('{nome_dip}', '{cogn_dip}', '{tel_dip}')"
    elif tab=='Z':
        query = f"INSERT INTO `zona_di_lavoro_marco_santini`(`nome`, `cognome`, `telefono`, `ore_lavoro`) VALUES ('{nome_dip}', '{cogn_dip}', '{tel_dip}', '{ore_dip}')"
    print(query)
    cur.execute(query)
    dati = conn.commit()
    return dati
...
def db_delete(data, tab):
    ...
    # si chiama una funzione di libreria passando i parametri di ricerca dell'utente. esempio controlla_caratteri(nome)
    if tab=='C':
        query = f"DELETE FROM clienti_marco_santini where nome= '{data}' "
    elif tab=='Z':
        query = f"DELETE FROM zona_di_lavoro_marco_santini where nome= '{data}' "
    print(query)
    cur.execute(query)
    conn.commit()
    return
...
def db_update(data, tab, par, mod):
    ...
    if tab=='C':
        if(par=="N"):
            query = f"UPDATE `clienti_marco_santini` SET `nome`='{mod}' WHERE 1=1 and nome='{data}'"
            cur.execute(query)
            conn.commit()
            return
        elif(par=="C"):
            query = f"UPDATE `clienti_marco_santini` SET `cognome`='{mod}' WHERE 1=1 and nome='{data}'"
            cur.execute(query)
            conn.commit()
            return
        elif(par=="T"):
            query = f"UPDATE `clienti_marco_santini` SET `telefono`='{mod}' WHERE 1=1 and nome='{data}'"
            cur.execute(query)
            conn.commit()
            return
    elif tab=='Z':
        if(par=="N"):
            query = f"UPDATE `zona_di_lavoro_marco_santini` SET `nome`='{mod}' WHERE 1=1 and nome='{data}'"
            cur.execute(query)
            conn.commit()
            return
        elif(par=="C"):
            query = f"UPDATE `zona_di_lavoro_marco_santini` SET `cognome`='{mod}' WHERE 1=1 and nome='{data}'"
            cur.execute(query)
            conn.commit()
            return
        elif(par=="T"):
            query = f"UPDATE `zona_di_lavoro_marco_santini` SET `telefono`='{mod}' WHERE 1=1 and nome='{data}'"
            cur.execute(query)
            conn.commit()
            return
        elif par=='L':
            query = f"UPDATE `zona_di_lavoro_marco_santini` SET `ore_lavoro`='{mod}' WHERE 1=1 and nome='{data}'"
            cur.execute(query)
            conn.commit()
            return
...
while True:
    ...
    thread.append(th.Thread(target=gestisci_comunicazione, args = (lista_connessioni[i][0],) )) 
    thread[i].start()
    i+=1
